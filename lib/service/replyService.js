...
const replyService = {
    ...
    reply_write_confirm: (req, res) => {
        ...
        let sql2 = `INSERT INTO TBL_REPLY(R_ORIGIN_NO, R_OWNER_STORY_NO, R_TXT, R_M_ID) VALUES (?, ?, ?, ?)`;
    ...
    /*
    reply_write_confirm: async function(req, res) {                            // 트랜젝션, inserId 이용
        ...
        DB.query(sql1, [], (error1, r_no) => {
            if (error1) {
                ...
                DB.query(sql2, [Number(r_no[0].R_NO + 1), post.s_no, post.r_txt, m_id], (error2, result) => {
                    if (error2) {
                        console.log('error2: ', error2);
                        res.json(null);
    ...
    */
    ...
    re_reply_write_confirm: (req, res) => {
        ...
        DB.query(sql1, [post.r_origin_no, post.r_owner_story_no, 1, post.r_txt, m_id], (error, result) => {
            if (error) {
    ...
    get_replys : (req, res) => {
        ...
        let sql2 = `SELECT COUNT(*) FROM TBL_REPLY WHERE R_ORIGIN_NO = ? AND R_CLASS = 1`;
    ...
    get_re_replys : (req, res) => {
        ...
        FROM TBL_REPLY 
        JOIN TBL_MEMBER ON TBL_REPLY.R_M_ID = TBL_MEMBER.M_ID 
        WHERE TBL_REPLY.R_ORIGIN_NO = ? AND R_CLASS = 1 
        ORDER BY TBL_REPLY.R_REG_DATE DESC`;
    ...
    modify_confirm: (req, res) => {
        ...
        // let post = req.body;
        let post = {
            'r_txt': '스토리 modify때 삭제된 사진은 로컬 폴더에서도 지워야 합니다.',
            'r_no' : 83
        }
    ...
    delete_confirm: (req, res) => {
        ...
        let sql1 = `UPDATE TBL_REPLY SET R_IS_DELETED = 1, R_DELETD_DATE = NOW() WHERE R_NO = ?`;
