...
const replyService = {
    ...
    */
    ...
    // 댓글 작성 컨펌 (insertId 이용)
    // reply_write_confirm: (req, res) => {                            // (트랜젝션은 락걸려 있어 불가능!!)
    ...
    reply_write_confirm: (req, res) => {                            // (트랜젝션은 락걸려 있어 불가능!!)
        ...
        let sql1 = `INSERT INTO TBL_REPLY(R_ORIGIN_NO, R_OWNER_STORY_NO, R_TXT, R_M_ID) VALUES(SELECT MAX(R_NO) + 1 FROM TBL_REPLY, ?, ?, ?)`;
    ...
    delete_confirm: (req, res) => {
        ...
        let sql1 = `UPDATE TBL_REPLY SET R_IS_DELETED = 1, R_DELETED_DATE = NOW() WHERE R_ORIGIN_NO = ?`;
        let sql2 = `UPDATE TBL_REPLY SET R_IS_DELETED = 1, R_DELETED_DATE = NOW() WHERE R_NO = ?`;

        switch (post.r_class) {

            case 0 :

                DB.query(sql1, [post.r_no], (error, result) => {
                    if (error) {
                        console.log('error: ', error);
                        res.json(null);
            ...
            case 1 :
                ...
                DB.query(sql2, [post.r_no], (error, result) => {
                    if (error) {
                        console.log('error: ', error);
                        res.json(null);
