...
const replyService = {
    ...
    /*
    reply_write_confirm: async function(req, res) {                            // 트랜젝션, inserId 이용 (lock 걸려 있어 불가능!!)
        ...
        let sql1 = `INSERT INTO TBL_REPLY(R_ORIGIN_NO, R_OWNER_STORY_NO, R_TXT, R_M_ID) VALUES(1, ?, ?, ?)`;
        ...
        try {
            ...
            let write1 = await connection1.query(sql1, [post.s_no, post.r_txt, post.m_id]);
            ...
            if (write1[0].affectedRows > 0) {
                ...
                try {
                    ...
                    if (write2[0].affectedRows > 0) {
                        connection1.commit();
                        connection2.commit();
                        res.json(write1[0].insertId);
                    } else {
                        connection1.rollback();
                        connection2.rollback();
                        res.json('write2 fail!!');
