    ...
    */
    ...
    reply_write_confirm: async function (req, res) {      
        ...
        try {
            ...
            let write1 = await connection.query(sql1, [post.s_no, post.r_txt, post.m_id]);

            if (write1[0].affectedRows > 0) {

                let write2 = await connection.query(sql2, [Number(write1[0].insertId), Number(write1[0].insertId)]);

                if (write2[0].affectedRows > 0) {
                    await connection.commit();
                    res.json(write2[0].affectedRows);
    ...
    re_reply_write_confirm: (req, res) => {
        ...
        DB.query(sql1, [post.r_origin_no, post.s_no, post.r_txt, post.m_id], (error, result) => {
            if (error) {
                console.log('error: ', error);
                res.json(null);
        ...
        switch (post.r_class) {
            ...
            case 1 :
                ...
                DB.query(sql2, [post.r_no], (error, result) => {
                    console.log('post.r_no: ', post.r_no);
                    if (error) {
                        console.log('error: ', error);
                        res.json(null);
