...
const storyService = {
    ...
    write_confirm: async function(req, res, savedDir) {
        ...
        let sql2 = `INSERT INTO TBL_STORY_PICTURE(SP_OWNER_NO, SP_PICTURE_NAME, SP_SAVE_DIR) VALUES(?, ?, ?)`;
    ...
    get_all_storys: (req, res) => {
        ...
        let sql6 = `SELECT COUNT(*) FROM TBL_REPLY WHERE R_OWNER_STORY_NO = ? AND R_IS_DELETED = 0`;
    ...
    modify_confirm : async function(req, res) {
        ...
        let delImg = curImg.filter(v => !keepImg.includes(v));      // 기존사진에서 유지하고 싶지 않은 사진들만 골라냈음!
        ...
        let sql1 = `UPDATE TBL_STORY SET S_TXT = ?, S_IS_PUBLIC = ?, S_MOD_DATE = NOW() WHERE S_NO = ?`;                // 스토리 내용 수정
        let sql2 = `UPDATE TBL_STORY_PICTURE SET SP_PICTURE_NAME = ?, SP_MOD_DATE = NOW() WHERE SP_PICTURE_NAME = ?`;   // 변경된 사진 사진 이름만 UPDATE
        let sql3 = `UPDATE TBL_STORY_PICTURE SET SP_IS_DELETED = 1 SP_MOD_DATE = NOW() WHERE SP_PICTURE_NAME = ?`;      // 삭제되는 사진들 SP_IS_DELETED = 1로 설정
        let sql4 = `INSERT INTO TBL_STORY_PICTURE(SP_OWNER_NO, SP_PICTURE_NAME) VALUES(?, ?)`;                          // 추가되는 사진들 INSERT
        ...
        try {
            ...
            if (modify1[0].affectedRows > 0) {
                ...
                try {
                    ...
                    if (newImg.length <= delImg.length) {

                        for (let i = 0; i < delImg.length; i++) {

                            console.log('newImg[i]: ', newImg[i]);
                            console.log('delImg[i]: ', delImg[i]);

                            if (newImg[i] == undefined) newImg[i] = '';

                            let modify2 = await connection2.query(sql2, [newImg[i], delImg[i]]);
    ...
    */
    ...
    // 스토리 삭제 컨펌 (transaction 사용) <--- 댓글과 사진이 없을때 오류남! 분기태워서 수정하기
    ...
    delete_confirm: async function(req, res) {
        ...
        let sql1 = `UPDATE TBL_STORY SET S_IS_DELETED = 0 WHERE S_NO = ?`;
    ...
    */
    ...
    // 스토리 삭제 컨펌 (DB.query 사용)
    /*
    delete_confirm: (req, res) => {
        printLog(DEFAULT_NAME, 'delete_confirm');

        let s_no = 2;         // req.body.s_no

        let sql1 = `UPDATE TBL_STORY SET S_IS_DELETED = 1, S_DELETD_DATE = NOW() WHERE S_NO = ?`;
        ...
        DB.query(sql1, [s_no], (error, result) => {
            if (error) {
                ...
                res.json({'result.affectedRows: ' : result.affectedRows});
