...
const storyService = {
    ...
    // 나 + 친구들의 모든 스토리 가져오기(홈 => 피드에 보이는 것)   // DB.query방식 (기존에 하던방식 => 시간소요 제일 적음)
    get_all_storys: (req, res) => {
        ...
        DB.query(sql1, m_id, (error, friends) => {
           if (error) {
            ...
            // append_sql += ` S_OWNER_ID = '${m_id}' `;
            
            for (let i = 0; i < Number(friends.length); i++) {
                append_sql += ` OR S_OWNER_ID = ? `;

            }

            sql2 = `SELECT * FROM TBL_STORY WHERE S_OWNER_ID = '${m_id}' ${append_sql} AND S_IS_DELETED = 0 ORDER BY S_REG_DATE DESC`;
