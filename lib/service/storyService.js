...
const storyService = {
    ...
    get_all_storys: (req, res) => {
        ...
        let sql2 = `SELECT * FROM TBL_STORY_PICTURE WHERE SP_OWNER_NO = ? AND SP_IS_DELETED = 0 ORDER BY SP_NO ASC`;
        let sql3 = `SELECT * FROM TBL_MEMBER WHERE M_ID = ?`;
        let sql4 = `SELECT COUNT(*) FROM TBL_STORY_LIKE WHERE SL_OWNER_STORY_NO = ? AND SL_IS_LIKE = 1`;
    ...
    modify_confirm : async function(req, res) {
        ...
        let sql2 = `UPDATE TBL_STORY_PICTURE SET SP_PICTURE_NAME = ?, SP_MOD_DATE = NOW() WHERE SP_NO = ?`;             // 변경된 사진 사진 이름만 UPDATE
        let sql3 = `UPDATE TBL_STORY_PICTURE SET SP_IS_DELETED = 1, SP_DELETED_DATE = NOW() WHERE SP_NO = ?`;           // 삭제되는 사진들 SP_IS_DELETED = 1로 설정
    ...
    */
    ...
    story_like_update: (req, res) => {
        ...
        let sql1 = `INSERT INTO TBL_STORY_LIKE (SL_OWNER_STORY_NO, SL_M_ID) VALUES (?, ?)`;
        let sql2 = `UPDATE TBL_STORY_LIKE SET SL_IS_LIKE = ${sl_update_num} WHERE SL_OWNER_STORY_NO = ? AND SL_M_ID = ?`
