...
const storyService = {
    ...
    write_confirm: async function(req, res, savedDir) {
        ...
        let insertStorySql = `INSERT INTO 
                        TBL_STORY(
                                    S_OWNER_ID, 
                                    S_TXT, 
                                    S_IS_PUBLIC) 
                        VALUES(?, ?, ?)`;                                       // 스토리 테이블에 작성자 ID, 작성 내용, 공개여부 INSERT
        let insertStoryPictruesSql = `INSERT INTO 
                        TBL_STORY_PICTURE(
                                            SP_OWNER_NO, 
                                            SP_PICTURE_NAME, 
                                            SP_SAVE_DIR) 
        ...
        try {
            ...
            let write1 = await connection.query(insertStorySql, [m_id, post.s_txt, post.s_is_public]);    // 스토리 테이블에 작성자 ID, 작성 내용, 공개여부 INSERT
            ...
            if (write1[0].affectedRows > 0) {
                ...
                let completedQueries = 0;                                                       // 여러번 요청 안보내게 completedQueries를 선언 하였음.
                    ...
                    let write2 = await connection.query(insertStoryPictruesSql, [write1[0].insertId, req.files[i].filename, savedDir]);     // 스토리 사진 테이블에 사진 INSERT
    ...
    get_all_storys: (req, res) => {
        ...
        let selectStoryPictures = `SELECT * FROM 
                        TBL_STORY_PICTURE 
                            WHERE 
                                SP_OWNER_NO = ? 
                                AND SP_IS_DELETED = 0 
                                ORDER BY SP_NO ASC`;                                                // 스토리 NO를 기반으로 스토리의 모든 사진들 가져오기
        let selectMemberInfos = `SELECT * FROM 
                        TBL_MEMBER 
                            WHERE 
                                M_ID = ?`;                                                          // 각각의 스토리의 memberInfors 가져오기
        let selectStoryLikeCnt = `SELECT COUNT(*) FROM 
                        TBL_STORY_LIKE 
                            WHERE 
                                SL_OWNER_STORY_NO = ? 
                                AND SL_IS_LIKE = 1`;                                                // 각각의 스토리의 좋아요 갯수 가져오기
        let selectStoryIsLike = `SELECT SL_IS_LIKE FROM 
                        TBL_STORY_LIKE 
                            WHERE 
                                SL_OWNER_STORY_NO = ? 
                                AND SL_M_ID = ?`;                                                   // 로그인 된 ID(본인)를 기준으로 스토리 좋아요 여부
        let selectStoryReplyCnt = `SELECT COUNT(*) FROM 
                        TBL_REPLY 
                            WHERE 
                                R_OWNER_STORY_NO = ? 
                                AND R_IS_DELETED = 0`;                                              // 각각의 스토리의 댓글 갯수 가져오기
        
        DB.query(selectStorys, [m_id, m_id, m_id], (error1, storys) => {
                ...
                if (error1) {
                    ...
                    for (let j = 0; j < storys.length; j++) {
                        ...
                        DB.query(selectStoryPictures, storys[j].S_NO, (error2, pictures) => {
                            if (error2) {
                                ...
                                DB.query(selectMemberInfos, storys[j].S_OWNER_ID, (error3, memberInfors) => {
                                    if (error3) {
                                        ...
                                        DB.query(selectStoryLikeCnt, storys[j].S_NO, (error4, likeCnt) => {
                                            if (error4) {
                                                ...
                                                DB.query(selectStoryIsLike, [storys[j].S_NO, m_id], (error5, isLike) => {
                                                    if (error5) {
                                                        ...
                                                        if (isLike[0] !== undefined) {
                                                            ...
                                                            DB.query(selectStoryReplyCnt, storys[j].S_NO, (error6, replysCnt) => {
                                                                if (error6) {
    ...
    */
    ...
    get_my_storys: async function(req, res) {
        ...
        if (logined_id !== m_id) {                                                          // 본인의 피드가 아닐 때는 비공개 게시물은 보이지 않음.
            appendSql+= ' AND (S_IS_PUBLIC = 0 OR S_IS_PUBLIC = 1)';

        }

        let selectStorys = `SELECT * FROM 
                        TBL_STORY 
                            WHERE 
                                S_OWNER_ID = ? 
        ...
        DB.query(selectStorys, m_id, (error1, storys) => {
                ...
                if (error1) {
                    ...
                    if (storys.length === 0) {
                        DB.query(selectMemberInfos, m_id, (error, memberInfors) => {
                            if (error) {
    ...
    get_story: async function(req, res) {
        ...
        DB.query(selectStory, s_no, (error1, story) => {
                ...
                if (error1) {
                    ...
                    res.json(null);
                        ...
                        DB.query(selectStoryPictures, story[0].S_NO, (error2, pictures) => {
                            if (error2) {
                                console.log('error2', error2);
                                return res.json(null);
                                ...
                                DB.query(selectMemberInfos, story[0].S_OWNER_ID, (error3, memberInfors) => {
                                    if (error3) {
                                        console.log('error3', error3);
                                        return res.json(null);
                                        ...
                                        DB.query(selectStoryLikeCnt, story[0].S_NO, (error4, likeCnt) => {
                                            if (error4) {
                                                console.log('error4', error4);
                                                return res.json(null);
                                                ...
                                                DB.query(selectStoryIsLike, [story[0].S_NO, m_id], (error5, isLike) => {
                                                    if (error5) {
                                                        console.log('error5', error5);
                                                        return res.json(null);
                                                        ...
                                                        if (isLike[0] !== undefined) {
                                                            ...
                                                            DB.query(selectStoryReplyCnt, story[0].S_NO, (error6, replysCnt) => {
                                                                if (error6) {
                                                                    console.log('error6',error6);
                                                                    return res.json(null);
    ...
    modify_confirm : async function(req, res) {
        ...
        try {
            ...
            let [savedDirQuery] = await connection.query(sql, [post.s_no]);
                    ...
                    if (newImg.length <= delImg.length) {                   // 새로 추가되는 사진의 길이가 삭제되는 사진의 갯수보다 작거나 같은 경우
                        ...
                        if (delImg.length === 0) {                                  // 삭제되는 사진이 없고 사진 추가만 되는경우!
                            ...
                            for (let j = 0; j < newImg.length; j++) {
                                ...
                                let modify4 = await connection.query(sql4, [post.s_no, newImg[j].filename, savedDir]);
