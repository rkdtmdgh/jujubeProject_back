...
const storyService = {
    ...
    modify_confirm : async function(req, res) {
        ...
        let newImg = ['a', 'd', 'e', 'f', 'g'];                         // req.file;

        let delImg = curImg.filter(v => !newImg.includes(v));
        let addImgPrototype = newImg.filter((v, i) => i >= curImg.length);
        let addImg = addImgPrototype.filter(v => !curImg.includes(v));
