...
const memberService = {
        ...
        try {
            ...
            try {
                ...
                for(let i = 0; i < sltMembers.length; i++) {
                    ...
                    let [result] = await conn2.query(sql2, [m_id, sltMembers[i].M_ID]);
                ...
                printLog(DEFAULT_NAME, 'get search member conn2 err', error);
            }

        } catch (error) {
            printLog(DEFAULT_NAME, 'get search member conn1 err', error);
        } 
    ...
    */
    ...
    friend_request_status: (req, res) => {
        ...
        DB.query(`
            SELECT FR_NO FROM TBL_FRIEND_REQUEST WHERE FR_REQ_ID = ? AND FR_RES_ID = ?
        `, [post.fr_req_id, post.fr_res_id], (err, result) => {
        ...
        try {
            ...
            if (result1.affectedRows > 0) {
                ...
                try {
                    ...
                    let [result2] = await conn2.query(sql2, [``, m_id, ``]);
