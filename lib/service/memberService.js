...
const memberService = {
    ...
    sign_in_confirm: async (req, res) => {
        ...
        try {
            ...
            let DuplicateLoginSql = `SELECT * FROM TBL_IS_LOGIN WHERE M_ID = ?`

            let [DuplicateLoginResult] = await conn.query(DuplicateLoginSql, [req.body.m_id]);

            if (DuplicateLoginResult.length > 0) {
                printLog(DEFAULT_NAME, '이미 로그인 중입니다.');
                return res.json({
                    result: -5,
                    message: '이미 로그인 중입니다.'
                });
    ...
    friend_request_confirm: async (req, res) => {
        ...
        try {
            ...
            if (insertResult.affectedRows > 0) {
                let [updateResult] = await conn.query(updateFriendRequestSql, [post.f_id, m_id]);
