...
const memberService = {
                    ...
                    `, [post.email], (err, member) => {
                        ...
                        // google mail과 일치하는 mail이 없는 경우
                            ...
                            DB.query(`
                                ...
                                VALUES(?, ?, ?, ?, ?, ?, ?)
                            `, [id, bcrypt.hashSync(pw, 10), post.name, post.email, '--', 'M', post.sub], (err3, result) => {
    ...
    get_search_member: async function(req, res) {
        ...
        let sql1 = `
            ...
            FROM 
                TBL_MEMBER 
            WHERE 
                M_IS_DELETED = 0 AND M_IS_SUSPEND = 0 AND M_ID LIKE ? ORDER BY M_ID ASC`;
            ...
            try {
                ...
                for(let i = 0; i < sltMembers.length; i++) {
                    ...
                    if (result.length <= 0) {
                        ...
                        sltMembers[i].F_ILCHON_NAME = result[i].F_ILCHON_NAME;
                    }
    ...
    */
            ...
            if(err1) {
                ...
                // 친구 신청 안한 경우 새로 신청
                    ...
                    DB.query(`
                        ...
                        VALUES(?, ?, ?)
                    `, [m_id, post.fr_res_id, post.fr_ilchon_name], (err, result) => {
    ...
    friend_request_confirm: async (req, res) => {
        ...
        let sql = `SELECT * FROM TBL_FRIEND WHERE F_OWNER_ID = ? AND F_ID = ?`;

        try {
            let [result] = await conn1.query(sql, [m_id, post.f_id]);
            printLog(DEFAULT_NAME, 'friend_request_confirm result', result);
            ...
            printLog(DEFAULT_NAME, 'friend_request_confirm sql error', error)
            return;
        }
        ...
        try {        // try 중첩 삭제하였음 (try 구문 안에서 하나의 트랜잭션이라도 오류가 나면 catch가 잡음!)

            // 로그인 user의 친구 목록에 수락한 친구 추가.
            let [result1] = await conn1.query(sql1, [m_id, post.f_id, post.f_request_ilchon_name]);

            // 친구 추가 성공 시.
            if (result1.affectedRows > 0) {
                printLog(DEFAULT_NAME, '친구추가1 성공!');

                    // 반대의 경우. 수락한 친구의 친구 목록에 로그인 user 추가.
                    let [result2] = await conn1.query(sql1, [post.f_id, m_id, post.f_response_ilchon_name]);

                    // 양쪽 친구 목록에 친구 추가 성공 시.
                    if (result2.affectedRows > 0) {
                        printLog(DEFAULT_NAME, '친구추가2 성공!');

                        // 친구 신청 테이블 친구 수락 여부 업데이트.
                        let [result3] = await conn1.query(sql2, [post.f_id, m_id]);

                        // 친구 신청 테이블 업데이트까지 업데이트 되면 커밋.
                        if (result3.affectedRows > 0) {
                            printLog(DEFAULT_NAME, '친구추가3 성공!');

                        await conn1.commit();
                        res.json({
                            result: result3.affectedRows
                        });

                        // 친구 신청 테이블 업데이트 실패시 롤백
                        } else {
                            await conn1.rollback();
                            printLog(DEFAULT_NAME, '친구추가3 실패!');
                            res.json({
    ...
    friend_delete_confirm: async (req, res) => {
        ...
        try {
            ...
            let [result1] = await conn1.query(sql1, [m_id, post.f_id]);

            if(result1.affectedRows > 0) {

                try {

                    let [result2] = await conn1.query(sql2, [post.f_id, m_id]);
    ...
    get_friend_list: async function (req, res) {
        ...
        try {
            let [friend_list] = await conn.query(sql1, [m_id]);

            if (friend_list.length <= 0) return res.json({friend_list: friend_list});

            for(let i = 0; i < friend_list.length; i++) {
                let [memberInfo] = await conn.query(sql2, [friend_list[i].F_ID]);
                friend_list[i].memberInfo = memberInfo;
            }

            res.json({friend_list : friend_list})

        } catch (error) {
            printLog(DEFAULT_NAME, 'get_friend_list select friend list error', error)
            res.json(null)
        }
    ...
    get_friend_request_list: (req, res) => {
        ...
        DB.query(`
            ...
            WHERE 
                FR_REQ_ID = ?
            `, [m_id], (err1, friend_request_list) => {

                if (err1) {
                    printLog(DEFAULT_NAME, 'get_friend_request_list err', err1);
                    return res.json(null);
                    ...
                    DB.query(`
                        ...
                        * 
                        ...
                        WHERE 
                            FR_RES_ID = ?
                        `, [m_id], (err2, friend_response_list) => {
                            if (err2) {
                                printLog(DEFAULT_NAME, 'get_friend_request_list err', err1);
                                return res.json(null);
                                ...
                                res.json({friend_request_list, friend_response_list});
